...
const ALIASES = { "copy": "copycat genetics", "copycat": "copycat genetics", "copycat genetics": "copycat genetics", "thc": "total health connections", "cwf": "cwf" };

function App() { const [selectedCase, setSelectedCase] = useState(''); const [data, setData] = useState([]); const [search, setSearch] = useState(''); const [error, setError] = useState(null); const [allData, setAllData] = useState([]);

useEffect(() => { async function fetchAll() { try { const all = await Promise.all( Object.entries(CASES).map(async ([label, url]) => { const res = await fetch(/api/seedlist?url=${encodeURIComponent(url)}); const text = await res.text(); const lines = text.split('\n').filter(line => line.trim().length > 0); return lines.map(line => ({ text: line, case: label })); }) ); setAllData(all.flat()); } catch (err) { setError({ error: true, message: err.message }); } } fetchAll(); }, []);
...
const normalizedSearch = search.toLowerCase().trim(); const alias = ALIASES[normalizedSearch] || normalizedSearch; const filtered = search ? allData.filter(entry => entry.text.toLowerCase().includes(alias)) : selectedCase ? data : [];

return ( <div style={{ padding: '1rem', fontFamily: 'Arial' }}> <h1>Chronic Seed Vault</h1> <
